...
(check-equal? (parse '{with {z = 9}
                            {y = 10}
                            {+ z y}})
              (AppC (LamC (list 'z 'y) (BinopC 'plus (IdC 'z) (IdC 'y))) (list (NumC 9) (NumC 10))))
...
(check-equal? (interp (ifC (IdC 'false) (IdC 'true) (BinopC 'plus (NumC 1) (NumC 2))) top-env) (NumV 3)) 
(check-equal? (interp (ifC (IdC 'true) (IdC 'false) (BinopC 'plus (NumC 1) (NumC 2))) top-env) (BooleanV 'false)) 
...
(check-equal? (interp (parse '{lam {a b} {+ a b}}) top-env)
              (CloV (list 'a 'b) (BinopC 'plus (IdC 'a) (IdC 'b)) top-env))
